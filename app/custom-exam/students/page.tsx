...
export default function CustomExamStudentsPage() {
  ...
  useEffect(() => {
    // Get standardName and sectionName from session storage for custom exam
  ...
  const handleProceedToExamCreation = () => {
    ...
    if (customExamGradeAndSection) {
      ...
      sessionStorage.setItem('customExamSelectedStudents', JSON.stringify(selectionData));
      router.push('/custom-exam/assign');
    }
  };
      ...
      <div className="space-y-6">
        <div className="bg-red-50 border-l-4 border-red-400 p-4 rounded-lg">
          ...
          <button
            type="button"
            onClick={() => router.back()}
            className="mt-4 px-4 py-2 rounded-lg text-[color:var(--primary-foreground)] bg-[color:var(--primary-500)] hover:bg-[color:var(--primary-600)]"
          >
            Go Back
          </button>
        </div>
