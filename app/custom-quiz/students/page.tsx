...
export default function CustomQuizStudentsPage() {
  ...
  useEffect(() => {
    // Get standardName and sectionName from session storage for custom quiz
  ...
  const handleProceedToQuizCreation = () => {
    ...
    if (customQuizGradeAndSection) {
      ...
      sessionStorage.setItem('customQuizSelectedStudents', JSON.stringify(selectionData));
      router.push('/custom-quiz/assign');
    }
  };
    ...
    <div className="pt-8 px-6 lg:px-16">
      ...
      {selectedStudents.length > 0 && (
        <div className="fixed bottom-6 right-6 z-50">
          <Button
            onClick={handleProceedToQuizCreation}
